...
func (c *controller) Eth1EventHandler(ongoingSync bool) eth1.SyncEventHandler {
	return func(e eth1.Event) ([]zap.Field, error) {
		...
		case abiparser.PodLiquidated:
			...
			return c.handlePodLiquidatedEvent(ev, ongoingSync)
		...
		case abiparser.FeeRecipientAddressAdded:
			...
			return c.handleFeeRecipientAddressAddedEvent(ev, ongoingSync)
		default:
...
func (c *controller) handleOperatorAddedEvent(event abiparser.OperatorAddedEvent) ([]zap.Field, error) {
	...
	if err := c.operatorsCollection.SaveOperatorData(od); err != nil {
...
func (c *controller) handleOperatorRemovedEvent(
	...
	shares, err := c.collection.GetFilteredValidatorShares(ByOperatorID(spectypes.OperatorID(event.Id)))
	if err != nil {
...
func (c *controller) handleValidatorAddedEvent(
	...
	if !found {
		validatorShare, err = c.onShareCreate(ValidatorAddedEvent)
		if err != nil {
	...
	if isOperatorShare {
		...
		if ongoingSync {
			...
			_, _ = c.onShareStart(validatorShare)
		}
	}
