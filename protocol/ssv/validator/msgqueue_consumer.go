...
func (v *Validator) processNoRunningInstance(q msgqueue2.MsgQueue, handler MessageHandler, msgID spectypes.MessageID, identifier string, lastHeight specqbft.Height) bool {
	...
	iterator := msgqueue2.NewIndexIterator().Add(func() msgqueue2.Index {
		return msgqueue2.SignedPostConsensusMsgIndex(identifier)
	}, func() msgqueue2.Index {
		return msgqueue2.DecidedMsgIndex(identifier)
	}, func() msgqueue2.Index {
		indices := msgqueue2.SignedMsgIndex(spectypes.SSVConsensusMsgType, identifier, lastHeight, false, specqbft.CommitMsgType)
		if len(indices) == 0 {
			return msgqueue2.Index{}
		}
		return indices[0]
	})
...
func (v *Validator) processByState(q msgqueue2.MsgQueue, handler MessageHandler, msgID spectypes.MessageID, identifier string, height specqbft.Height) bool {
	runner := v.DutyRunners.DutyRunnerForMsgID(msgID)
...
func (v *Validator) processHigherHeight(q msgqueue2.MsgQueue, handler MessageHandler, identifier string, lastHeight specqbft.Height, higherCache *cache.Cache) bool {
	msgs := q.WithIterator(1, true, func(index msgqueue2.Index) bool {
		key := index.String()
...
func (v *Validator) processLateCommit(q msgqueue2.MsgQueue, handler MessageHandler, identifier string, lastHeight specqbft.Height) bool {
	iterator := msgqueue2.NewIndexIterator().
		Add(func() msgqueue2.Index {
			indices := msgqueue2.SignedMsgIndex(spectypes.SSVConsensusMsgType, identifier, lastHeight-1, false, specqbft.CommitMsgType)
			if len(indices) == 0 {
				return msgqueue2.Index{}
			}
			return indices[0]
		...
		Add(func() msgqueue2.Index {
			indices := msgqueue2.SignedMsgIndex(spectypes.SSVConsensusMsgType, identifier, lastHeight-1, true, specqbft.CommitMsgType)
			if len(indices) == 0 {
				return msgqueue2.Index{}
			}
			return indices[0]
...
func (v *Validator) getNextMsgForState(q msgqueue2.MsgQueue, identifier string, height specqbft.Height) *spectypes.SSVMessage {
	iterator := msgqueue2.NewIndexIterator()

	idxs := msgqueue2.SignedMsgIndex(spectypes.SSVConsensusMsgType, identifier, height, false,
