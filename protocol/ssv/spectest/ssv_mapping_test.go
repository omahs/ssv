...
func fixRunnerForRun(t *testing.T, baseRunner map[string]interface{}, ks *testingutils.TestKeySet) runner2.Runner {
	base := &runner2.BaseRunner{}
...
func fixControllerForRun(t *testing.T, runner runner2.Runner, contr *controller.Controller, ks *testingutils.TestKeySet) *controller.Controller {
...
func baseRunnerForRole(role spectypes.BeaconRole, base *runner2.BaseRunner, ks *testingutils.TestKeySet) runner2.Runner {
	switch role {
	case spectypes.BNRoleAttester:
		ret := ssvtesting.AttesterRunner(ks)
		ret.(*runner2.AttesterRunner).BaseRunner = base
		return ret
	case spectypes.BNRoleAggregator:
		ret := ssvtesting.AggregatorRunner(ks)
		ret.(*runner2.AggregatorRunner).BaseRunner = base
		return ret
	case spectypes.BNRoleProposer:
		ret := ssvtesting.ProposerRunner(ks)
		ret.(*runner2.ProposerRunner).BaseRunner = base
		return ret
	case spectypes.BNRoleSyncCommittee:
		ret := ssvtesting.SyncCommitteeRunner(ks)
		ret.(*runner2.SyncCommitteeRunner).BaseRunner = base
		return ret
	case spectypes.BNRoleSyncCommitteeContribution:
		ret := ssvtesting.SyncCommitteeContributionRunner(ks)
		ret.(*runner2.SyncCommitteeAggregatorRunner).BaseRunner = base
		return ret
